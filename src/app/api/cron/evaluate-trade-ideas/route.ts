 ...
 *
 * Mirrors scripts/evaluate-trade-ideas.mjs but server-side. Picks expired
 * (or active-but-validUntil-passed) trade ideas with no result.outcome,
 * fetches Binance 1H klines between publishedAt~validUntil, judges SL/TP1
 * touch, and patches result + status.
 */
...
export async function GET(req: Request) {
  ...
  return NextResponse.json({ ok: true, count: ideas.length, results });
}
